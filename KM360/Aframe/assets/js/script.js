...
for (let i = 0; i <= 5; i++) {
    $(document).on('click', '#addCircle' + i, function () {
        if (!$("#room-name" + i).val()) {
            $("#north-error"+i).removeClass("css--display-none");
            $("#north-error"+i).html("<div>Telpas nosaukums nav pievienots</div>")
        } else if (!$("#room-size" + i).val()) {
            $("#north-error"+i).removeClass("css--display-none");
            $("#north-error"+i).html("<div>Telpas platība nav pievienota</div>")
        } else if (!$("#panorama" + i).val()) {
            $("#north-error"+i).removeClass("css--display-none");
            $("#north-error"+i).html("<div>Telpas panorāma nav pievienota</div>")
        } else {
            $("#north-error"+i).addClass("css--display-none");
            ...
            var toInsert = '<div id="pointer' + i + '" class="flag" style="left:' + 60 * (i - 1) + 'px" >' +
                ...
                '<circle  data-size="' + rSize + '" data-name="' + rName + '" data-north="' + false + '" class="circle" style="cursor: pointer;" id="pointerSVG' + i + '" cx="500" cy="500" r="227.5" fill="#494949" stroke="black" stroke-width="45"/></svg></div>';
...
$(document).ready(function () {
    ...
    $("#addButton").click(function () {
        ...
        newTextBoxDiv.after().html(
            ...
            '<button id="btn-close-'+counter+'" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>\n' +
            '</div>' +
            '<div class="modal-body">' +
            '<div class="modal-body row">' +
            '<div class="col-6"><input  class="form-control form-styled" id="room-name' + counter + '" type="text" placeholder="Nosaukums"> </div>' +
            '<div class="col-6"><input  class="form-control form-styled" id="room-size' + counter + '" type="number" placeholder="Platība"> </div>' +
            '<div class="col-12 d-flex justify-content-center position-relative">' +
            '<input class="upload-styled-input" type="file" id="panorama' + counter + '" accept=".png, .jpg, .jpeg, .svg" onchange="encodeImageFileAsURL(this)">' +
            '<label class="upload-styled-label" for="panorama1">Pievieno panorāmu</label>' +
            '</div> ' +
            '<div id="north-error'+counter+'" class="alert alert-danger mt-3 css--display-none" role="alert"></div>' +
            ...
            '<div class="col-6"> ' +
            '<button type="button" class="btn btn-styled" data-bs-toggle="modal" data-bs-target="#PanoModal' + counter + '">' +
...
function createJSON() {
    ...
    let counter = 0;
    for (let x = 1; x <= 5; x++) {
        if ($('#pointerSVG' + x).data("north") === false) {
            errorsNorth[counter] = x;
            counter++;
        }
    }
    ...
    let jsonObj = {};
    let i = 0;
    if (errorsNorth.length > 0) {
        $("#north-error").removeClass("css--display-none")
        let errorText = '';
        console.log(errorsNorth)
        $.map(errorsNorth, function (val) {
            errorText += "<div>Panorāmai Nr." + val + " nav uzstādīti ziemeļi</div>"
            console.log(val)
        });
        $("#north-error").html(errorText)
    } else {
        $("#north-error").addClass("css--display-none")
...
$("#compass-set-button").click(function () {
    ...
    $('#pointerSVG' + (activeRoom + 1)).data("north", true)
});
